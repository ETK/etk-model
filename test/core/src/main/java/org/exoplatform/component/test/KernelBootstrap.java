...
public class KernelBootstrap
{
  ...
  public KernelBootstrap(ClassLoader realClassLoader) {
    ...
    // due to the maven is automatic assign the System.property basedir. Only
    // run by maven for unit testing
  ...
  /**
   * Set the tmp dir of the test to a new file location. The new tmp dir must be
   * a strict descendant of the {@link #targetDir} file.
   * 
   * @param tmpDir the new tmp dir
   * @throws IllegalArgumentException if the tmp dir is not a descendant of the
   *           target dir
   */
  ...
  public void boot() throws IllegalStateException {
    ...
    try {
      // Must clear the top container first otherwise it's not going to work
      // well
